...
PARAM_TOOLTIPS = {
    ...
    # ── Aurora Borealis ──
    ("aurora_config", "FPS"): "Animation frame rate for the aurora simulation.",
    ("aurora_config", "SCROLL_SPEED"): "How fast the aurora bands drift vertically across the keyboard.",
    ("aurora_config", "CURTAIN_FREQ"): "Spatial frequency of the curtain fold pattern. Higher values create tighter folds.",
    ("aurora_config", "CURTAIN_SPEED"): "Animation speed of the curtain undulation.",
    ("aurora_config", "FOLD_FACTOR"): "Per-row phase offset for the curtain fold. Creates the draped fabric look.",
    ("aurora_config", "NOISE_SCALE"): "Sampling scale for the noise texture. Larger values create finer detail.",
    ("aurora_config", "BRIGHTNESS"): "Overall brightness multiplier for the aurora bands.",
    ("aurora_config", "BG_COLOR"): "Dark sky background color.",
    ("aurora_config", "BAND_1_COLOR"): "Color of the first (green) aurora band.",
    ("aurora_config", "BAND_1_ROW"): "Vertical center of the first band (row units).",
    ("aurora_config", "BAND_1_WIDTH"): "Vertical spread of the first band (Gaussian sigma).",
    ("aurora_config", "BAND_2_COLOR"): "Color of the second (cyan) aurora band.",
    ("aurora_config", "BAND_2_ROW"): "Vertical center of the second band.",
    ("aurora_config", "BAND_2_WIDTH"): "Vertical spread of the second band.",
    ("aurora_config", "BAND_3_COLOR"): "Color of the third (magenta) aurora band.",
    ("aurora_config", "BAND_3_ROW"): "Vertical center of the third band.",
    ("aurora_config", "BAND_3_WIDTH"): "Vertical spread of the third band.",
    ("aurora_config", "STAR_CHANCE"): "Probability of a star twinkle appearing in dark regions per pixel per frame.",
    ("aurora_config", "STAR_COLOR"): "Color of star twinkle flashes.",

    # ── Boid Flock ──
    ("boids_config", "FPS"): "Animation frame rate for the flocking simulation.",
    ("boids_config", "NUM_BOIDS"): "Number of flocking agents. More boids create denser swarms.",
    ("boids_config", "VISUAL_RANGE"): "How far each boid can see neighbors (in key units). Controls flock cohesion radius.",
    ("boids_config", "SEPARATION_DIST"): "Minimum comfortable distance between boids. Closer boids repel each other.",
    ("boids_config", "W_SEPARATION"): "Strength of the separation force. Higher values keep boids farther apart.",
    ("boids_config", "W_ALIGNMENT"): "Strength of velocity alignment with neighbors. Higher values make tighter flocks.",
    ("boids_config", "W_COHESION"): "Strength of attraction toward the flock center. Pulls stragglers in.",
    ("boids_config", "W_WALL"): "Strength of boundary avoidance. Keeps boids from flying off the keyboard edge.",
    ("boids_config", "MARGIN"): "Distance from keyboard edge where wall avoidance activates (key units).",
    ("boids_config", "MIN_SPEED"): "Minimum boid velocity. Prevents boids from stalling.",
    ("boids_config", "MAX_SPEED"): "Maximum boid velocity. Caps how fast boids can fly.",
    ("boids_config", "TRAIL_DEPOSIT"): "Brightness deposited by each boid per frame. Higher values leave brighter trails.",
    ("boids_config", "TRAIL_DECAY"): "Trail fade rate per frame (0-1). Lower values make trails disappear faster.",
    ("boids_config", "ASPECT_RATIO"): "Keyboard aspect ratio correction (width/height). Adjusts distance calculations.",
    ("boids_config", "STARTLE_INTERVAL_MIN"): "Minimum frames between startle events that scatter the flock.",
    ("boids_config", "STARTLE_INTERVAL_MAX"): "Maximum frames between startle events.",
    ("boids_config", "STARTLE_IMPULSE"): "Velocity impulse applied during a startle event. Higher values scatter more violently.",
    ("boids_config", "PALETTE"): "Trail color gradient from dark (no trail) to bright (fresh trail).",

    # ── Heartbeat Pulse ──
    ("heartbeat_config", "FPS"): "Animation frame rate for the heartbeat effect.",
    ("heartbeat_config", "BPM"): "Base heart rate in beats per minute.",
    ("heartbeat_config", "BPM_MIN"): "Minimum BPM during the slow phase of the breathing cycle.",
    ("heartbeat_config", "BPM_MAX"): "Maximum BPM during the fast phase of the breathing cycle.",
    ("heartbeat_config", "BPM_CYCLE"): "Duration of one full BPM oscillation cycle (seconds).",
    ("heartbeat_config", "EXPAND_SPEED"): "How fast pulse rings expand outward from the center.",
    ("heartbeat_config", "RING_WIDTH"): "Thickness of each pulse ring (Gaussian sigma in key units).",
    ("heartbeat_config", "LUB_STRENGTH"): "Intensity of the first heartbeat pulse (lub).",
    ("heartbeat_config", "DUB_STRENGTH"): "Intensity of the second heartbeat pulse (dub). Usually weaker than lub.",
    ("heartbeat_config", "DUB_PHASE"): "Timing of the dub pulse as a fraction of the beat cycle (0-1).",
    ("heartbeat_config", "CENTER_ROW"): "Vertical center of the heartbeat origin (row units).",
    ("heartbeat_config", "CENTER_COL"): "Horizontal center of the heartbeat origin (column units).",
    ("heartbeat_config", "ASPECT_RATIO"): "Keyboard aspect ratio correction for circular pulse rings.",
    ("heartbeat_config", "VEIN_BRIGHTNESS"): "Base brightness of the vein network pattern between pulses.",
    ("heartbeat_config", "VEIN_PULSE"): "How much veins brighten during a heartbeat pulse.",
    ("heartbeat_config", "SYSTOLE_FLUSH"): "Intensity of the whole-keyboard warm flush during systole contraction.",
    ("heartbeat_config", "PALETTE"): "Cardiac color gradient: dark purple through deep red to warm pink.",

    # ── Ember Drift ──
    ("ember_config", "FPS"): "Animation frame rate for the ember particle simulation.",
    ("ember_config", "NUM_EMBERS"): "Maximum number of ember particles active at once.",
    ("ember_config", "SPAWN_RATE"): "Rate of new ember spawning per frame. Higher values create denser streams.",
    ("ember_config", "RISE_SPEED"): "How fast embers rise upward (rows per frame).",
    ("ember_config", "RISE_JITTER"): "Random variation in rise speed per frame. Adds organic irregularity.",
    ("ember_config", "WIND_STRENGTH"): "Amplitude of the horizontal wind sway.",
    ("ember_config", "WIND_SPEED"): "How fast the wind direction oscillates.",
    ("ember_config", "WIND_PHASE"): "Per-row phase offset for wind. Creates a swirling, layered wind effect.",
    ("ember_config", "SPARK_CHANCE"): "Probability per frame of spawning a bright, short-lived spark particle.",
    ("ember_config", "MAX_AGE"): "Maximum lifetime of an ember in frames before it fades out.",
    ("ember_config", "BED_GLOW"): "Brightness of the glowing ember bed along the bottom rows.",
    ("ember_config", "PALETTE"): "Ember cooling gradient: black → red → orange → yellow → white hot.",

    # ── Raindrop Ripples ──
    ("raindrop_config", "FPS"): "Animation frame rate for the raindrop simulation.",
    ("raindrop_config", "SPAWN_CHANCE"): "Probability of a new raindrop per frame. Higher values mean more frequent drops.",
    ("raindrop_config", "MAX_RIPPLES"): "Maximum number of simultaneous ripples on the surface.",
    ("raindrop_config", "EXPAND_SPEED"): "How fast ripple rings expand outward from the impact point.",
    ("raindrop_config", "RING_FREQ"): "Number of concentric rings per ripple. Higher values create tighter ring patterns.",
    ("raindrop_config", "RING_WIDTH"): "Width of the ring envelope (Gaussian sigma). Controls how quickly rings fade at edges.",
    ("raindrop_config", "DECAY_RATE"): "How fast ripples lose amplitude over time. Higher values mean shorter-lived ripples.",
    ("raindrop_config", "MAX_AGE"): "Maximum lifetime of a ripple in frames.",
    ("raindrop_config", "ASPECT_RATIO"): "Keyboard aspect ratio correction for circular ripples.",
    ("raindrop_config", "SPLASH_FRAMES"): "Duration of the bright splash flash at the impact point (frames).",
    ("raindrop_config", "SPLASH_COLOR"): "Color of the initial splash highlight when a drop hits.",
    ("raindrop_config", "PALETTE"): "Water surface color gradient from dark depths to bright ripple crests.",

    # ── Nebula Clouds ──
    ("nebula_config", "FPS"): "Animation frame rate for the nebula effect.",
    ("nebula_config", "NOISE_SCALE_1"): "Sampling scale for the warm nebula layer. Larger values create finer cloud detail.",
    ("nebula_config", "NOISE_SCALE_2"): "Sampling scale for the cool accent layer.",
    ("nebula_config", "DRIFT_SPEED_1"): "Drift speed of the warm nebula layer. Controls how fast clouds move.",
    ("nebula_config", "DRIFT_SPEED_2"): "Drift speed of the cool accent layer. Different from layer 1 for parallax depth.",
    ("nebula_config", "DRIFT_ANGLE_1"): "Drift direction of the warm layer (degrees). Creates directional cloud flow.",
    ("nebula_config", "DRIFT_ANGLE_2"): "Drift direction of the cool layer (degrees). Offset from layer 1 for parallax.",
    ("nebula_config", "ACCENT_STRENGTH"): "Blend strength of the cool accent layer (0 = hidden, 1 = full).",
    ("nebula_config", "BRIGHTNESS"): "Overall brightness multiplier for the nebula.",
    ("nebula_config", "STAR_CHANCE"): "Probability of a star birth flash per frame in bright nebula regions.",
    ("nebula_config", "STAR_THRESHOLD"): "Minimum nebula brightness for star birth to occur (0-1).",
    ("nebula_config", "STAR_COLOR"): "Color of newborn star flashes.",
    ("nebula_config", "PALETTE_WARM"): "Warm nebula color gradient: black → purple → magenta → pink.",
    ("nebula_config", "PALETTE_COOL"): "Cool accent color gradient: black → dark blue → teal.",

    # ── Corrupt ──
    ("corrupt_config", "FPS"): "Animation frame rate for the corruption patch effect.",
    ("corrupt_config", "IDLE_COLOR"): "Dark baseline color shown between corruption patches.",
    ("corrupt_config", "SPAWN_CHANCE"): "Probability per frame of spawning a new corruption patch.",
    ("corrupt_config", "MAX_PATCHES"): "Maximum number of simultaneous corruption zones on screen.",
    ("corrupt_config", "PATCH_WIDTH_MIN"): "Minimum width of a corruption patch (columns).",
    ("corrupt_config", "PATCH_WIDTH_MAX"): "Maximum width of a corruption patch (columns).",
    ("corrupt_config", "PATCH_HEIGHT_MIN"): "Minimum height of a corruption patch (rows).",
    ("corrupt_config", "PATCH_HEIGHT_MAX"): "Maximum height of a corruption patch (rows).",
    ("corrupt_config", "PATCH_LIFE_MIN"): "Minimum lifetime of a corruption patch (frames).",
    ("corrupt_config", "PATCH_LIFE_MAX"): "Maximum lifetime of a corruption patch (frames).",
    ("corrupt_config", "GLITCH_COLORS"): "Cyberpunk color palette used for corrupted pixels within patches.",

    # ── Corruption ──
    ("corruption_config", "FPS"): "Animation frame rate for the organic corruption simulation.",
    ("corruption_config", "BASELINE_PALETTE"): "Breathing gradient colors for the healthy baseline state.",
    ("corruption_config", "BASELINE_SPEED"): "Speed of the baseline breathing gradient animation.",
    ("corruption_config", "SPAWN_INTERVAL"): "Average seconds between new corruption site spawns.",
    ("corruption_config", "MAX_SITES"): "Maximum number of simultaneous corruption zones.",
    ("corruption_config", "RADIUS_MIN"): "Minimum blob radius for a corruption site (key units).",
    ("corruption_config", "RADIUS_MAX"): "Maximum blob radius for a corruption site (key units).",
    ("corruption_config", "INCUBATION_TIME"): "Duration of the initial single-pixel flicker warning phase (seconds).",
    ("corruption_config", "SPREAD_TIME"): "Duration of the growth phase where the blob expands (seconds).",
    ("corruption_config", "PEAK_TIME_MIN"): "Minimum duration of the peak corruption phase (seconds).",
    ("corruption_config", "PEAK_TIME_MAX"): "Maximum duration of the peak corruption phase (seconds).",
    ("corruption_config", "DECAY_TIME"): "Duration of the contraction/healing phase (seconds).",
    ("corruption_config", "SCAR_TIME"): "Duration of the residual discoloration after healing (seconds).",
    ("corruption_config", "CORRUPT_COLOR"): "Hot magenta-pink color of active corruption.",
    ("corruption_config", "SCAR_COLOR"): "Subtle green tint for healed/scarred areas.",
    ("corruption_config", "CASCADE_CHANCE"): "Probability of spawning child corruption sites when a site reaches peak.",
    ("corruption_config", "SURGE_INTERVAL"): "Seconds between power-surge flares that amplify all active corruption.",
    ("corruption_config", "ROW_BLEED_CHANCE"): "Probability per frame of a full-width scanline bleed from high-intensity zones.",

    # ── Voronoi Shatter ──
    ("voronoi_config", "FPS"): "Animation frame rate for the Voronoi diagram animation.",
    ("voronoi_config", "NUM_SEEDS"): "Number of Voronoi seed points. More seeds create more, smaller cells.",
    ("voronoi_config", "SEED_SPEED"): "Base drift speed of the seed points.",
    ("voronoi_config", "SEED_JITTER"): "Random Brownian jitter added to seed movement each frame.",
    ("voronoi_config", "SEED_REPULSION"): "Strength of repulsive force between nearby seeds. Prevents clustering.",
    ("voronoi_config", "HUE_SPEED"): "Speed of hue rotation for cell colors. Creates shifting color patterns.",
    ("voronoi_config", "EDGE_WIDTH"): "Width of the glowing edges between Voronoi cells.",
    ("voronoi_config", "EDGE_COLOR"): "Neon glow color of cell boundaries.",
    ("voronoi_config", "EDGE_BRIGHTNESS"): "Brightness multiplier for cell edges (0-1).",
    ("voronoi_config", "CELL_BRIGHTNESS"): "Brightness of cell fill colors (0-1). Lower values make edges stand out more.",
    ("voronoi_config", "ASPECT_RATIO"): "Keyboard aspect ratio correction for distance calculations.",
    ("voronoi_config", "SHATTER_INTERVAL_MIN"): "Minimum frames between shatter events that split or merge seeds.",
    ("voronoi_config", "SHATTER_INTERVAL_MAX"): "Maximum frames between shatter events.",
    ("voronoi_config", "SHATTER_DURATION"): "Duration of a shatter event animation (frames).",
}
